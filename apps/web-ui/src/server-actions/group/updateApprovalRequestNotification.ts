...
export type UpdateApprovalRequestNotificationState = {
...
export async function updateApprovalRequestNotificationSubmit(prevState: UpdateApprovalRequestNotificationState | undefined, formData: FormData) {
  ...
  logger.info("updateApprovalRequestNotificationSubmit:sessionUser", sessionUser);
  logger.info("updateApprovalRequestNotificationSubmit:notificationTypeId", notificationTypeId);
  logger.info("updateApprovalRequestNotificationSubmit:groupId", groupId);
  logger.info("updateApprovalRequestNotificationSubmit:approvalRequestNotificationId", approvalRequestNotificationId);
  ...
  try {
    if (approvalRequestNotificationId) {
      // Group member notification exists, update it.
      await stampHubClient.userRequest.group.updateApprovalRequestNotification.mutate({
      ...
      await stampHubClient.userRequest.group.createApprovalRequestNotification.mutate({
        requestUserId: sessionUser.stampUserId,
        groupId: groupId.toString(),
        notificationChannel: {
          id: channelConfigPropertyValues["channelId"].toString(),
