...
export async function approveSubmit(prevState: ApproveState | undefined, formData: FormData) {
  ...
  try {
    await stampHubClient.userRequest.approvalRequest.approve.mutate({
      approvalRequestId: approvalRequestId.toString(),
      approvedComment: comment.toString(),
      userIdWhoApproved: sessionUser.stampUserId,
    });
