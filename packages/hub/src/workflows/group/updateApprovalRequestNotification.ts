...
export const updateApprovalRequestNotification =
  (
    logger: Logger,
  ...
  (input: UpdateApprovalRequestNotificationInput): ResultAsync<Group, StampHubError> => {
    ...
    return parseZodObjectAsync(input, UpdateApprovalRequestNotificationInput)
      ...
      .andThen(({ group, notificationConfig, user, input }) => {
        const message = `${user.userName}(${user.email}) update approval request notification for ${group.groupName}`;
        return notificationConfig.handlers.setChannel({ properties: input.notificationChannel.properties, message }).andThen(() => {
          return okAsync(input);
        });
      })
      .andThen(groupProvider.updateApprovalRequestNotification)
