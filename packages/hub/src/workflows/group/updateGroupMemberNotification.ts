...
export const updateGroupMemberNotification =
  (
    logger: Logger,
  ...
  (input: UpdateGroupMemberNotificationInput): ResultAsync<Group, StampHubError> => {
    ...
    return parseZodObjectAsync(input, UpdateGroupMemberNotificationInput)
      ...
      .andThen(({ group, notificationConfig, user, input }) => {
        const message = `${user.userName}(${user.email}) update group member notification for ${group.groupName}`;
        return notificationConfig.handlers.setChannel({ properties: input.notificationChannel.properties, message }).andThen(() => {
          return okAsync(input);
        });
      })
      .andThen(groupProvider.updateGroupMemberNotification)
