...
export const deleteApprovalRequestNotification =
  (
    logger: Logger,
  ...
  (input: DeleteApprovalRequestNotificationInput): ResultAsync<Group, StampHubError> => {
    ...
    return parseZodObjectAsync(input, DeleteApprovalRequestNotificationInput)
      ...
      .andThen(({ group, input }) => {
        if (!group.approvalRequestNotifications || group.approvalRequestNotifications.length === 0) {
          return errAsync(
            new StampHubError("Target approvalRequestNotifications does not exist", "Target approvalRequestNotifications does not exist", "BAD_REQUEST")
          );
        }
        const targetGroupNotification = group.approvalRequestNotifications.find((notification) => notification.id === input.notificationId);
        if (!targetGroupNotification) {
          return errAsync(
            new StampHubError(
              "Target notification ID not found in approvalRequestNotifications",
              "Target notification ID not found in approvalRequestNotifications",
              "BAD_REQUEST"
            )
          );
      ...
      .andThen(({ group, notificationConfig, targetGroupNotification, user, input }) => {
        const message = `${user.userName}(${user.email}) delete approval request notification for ${group.groupName}`;
        return notificationConfig.handlers.unsetChannel({ id: targetGroupNotification.notificationChannel.id, message }).andThen(() => {
          return okAsync(input);
        });
      })
      .andThen(groupProvider.deleteApprovalRequestNotification)
