...
import { checkCanEditResourceImpl } from "./canEditResource";
...
describe("Testing canEditResource", () => {
  describe("checkCanEditResourceImpl", () => {
    ...
    it("should return ok with valid input", async () => {
      ...
      const result = await checkCanEditResourceImpl(
        input,
    ...
    it("should return error if handler returns none", async () => {
    ...
    it("should return error if DB providers return none", async () => {
      const getCatalogDBProviderNone = vi.fn().mockReturnValue(okAsync(none));
    ...
    it.each([
      ...
      const result = await checkCanEditResourceImpl(input, fn1, fn2, fn3, fn4);
